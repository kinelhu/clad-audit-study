...
save_df_tbl(out, "T_trials_detail", group_col = "Intent", landscape = TRUE,
  ...
  footer = sprintf(paste0(
    ...
    "of a trial reported elsewhere in this table, so the %d reports correspond to %d distinct randomized ",
    "trials. Trial protocol papers report no results and are outside the audit population, so they do not ",
    "appear here. Of the %d reports addressing established ",
